...
<?php
	...
	echo '<img src="defaults/gaming_memes.png" style="width=200px;height:200px;" id="group_dp">';
	echo '<h4 class="group-name" style="margin-left:65px;">Gaming Memes</h4>';

	$sql="SELECT totalSubscribers,totalMemesForThisCategory FROM memecategoriestable WHERE memeCategory='Gaming'";
	$result=mysqli_query($conn,$sql);
	$row=mysqli_fetch_assoc($result);
	$memeCategorySubscribers=$row['totalSubscribers'];
	$totalMemesForThisCategory=$row['totalMemesForThisCategory'];

	$memeCategorySubscribersLabel="categorySubscribeGaming";
	...
	if(isset($_SESSION['id'])){
		...
		$sql6= "SELECT subscribedByUserId FROM gamingmemessubscriberstable WHERE subscribedByUserId='$userId'";
	...
	echo '<button type="submit" onclick="memeCategorySubscribeFunction(0,\'Gaming\',\''.$memeCategorySubscribersLabel.'\');" name="subscribecategoryGaming" class="btn" id="userprofile-subscribe-button" style="margin-top:50px;margin-left:10px">'.$subscribeToMemeCategoryInnerHtml.'</button>';
	echo '</div>';
?>
</div>
<div class="tab-content" id="tabs">
	...
	<div class="tab trendingmemes" id="trending" style="right:400px;top:120px">
		<div id="trending_content_div">
			<?php
			$sql="SELECT * FROM memestable WHERE (memeCategory='Gaming' AND (visibilityStatus=1 OR visibilityStatus=3)) ORDER BY likes DESC";
			$result=mysqli_query($conn,$sql);
			...
			foreach($gaming_trending as $key => $am){
				...
				$gaming_trending[$key]['trending_score']=$score;			
			}

			//removing all duplicate values
			$gaming_trending = removeDuplicateArrayEntry($gaming_trending, 'id');

			//sorting in descending order according to the score
			usort($gaming_trending, function ($a, $b) {
			...
			//print_r($gaming_trending);

			$_SESSION['gaming_page_trending_counter']=0;
			$numberOfMemesInTrending=count($gaming_trending);			
			//echo $numberOfMemesInTrending;
			loadMorefunction($gaming_trending,$numberOfMemesInTrending,'gaming_page_trending_counter','trending',"donthideform");

			echo '</div>';//closing trending_content_div

			//load more button
			if($numberOfMemesInTrending!=$_SESSION['gaming_page_trending_counter'])
			{
				echo '
				<form class="load_more_memes_form" id="gaming_page_trending_loadmore_form">
					<input type="hidden" name="load_more_type" value="trending">
					<input type="hidden" name="session_counter_name" value="gaming_page_trending_counter">
					<input type="hidden" name="numberOfElementsInArray" value="'.$numberOfMemesInTrending.'">';?>
					<input type="hidden" name="data_array" value='<?php echo base64_encode(serialize($gaming_trending)) ; ?>'>
				<?php
				echo '
	...
	<div class="tab freshmemes" id="fresh" style="right:400px;top:120px">
		...
		<?php
		$sql= "SELECT * FROM memestable WHERE memeCategory='Gaming' AND (visibilityStatus=1 OR visibilityStatus=3) ORDER BY id DESC";//ordering in the descending order(that is, printing all the memes in the decreasing order of their id)
		$result=mysqli_query($conn,$sql);
		...
		if(isset($fresh_gaming)){

		$_SESSION['gaming_page_fresh_counter']=0;
		$numberOfMemesInFresh=count($fresh_gaming);				
		loadMorefunction($fresh_gaming,$numberOfMemesInFresh,'gaming_page_fresh_counter','fresh',"donthideform");

		echo '</div>';//closing fresh_content_div

		//load more button
		if($numberOfMemesInFresh!=$_SESSION['gaming_page_fresh_counter'])
		{
			echo '
			<form class="load_more_memes_form" id="gaming_page_fresh_loadmore_form" method="post">
				<input type="hidden" name="load_more_type" value="fresh">
				<input type="hidden" name="session_counter_name" value="gaming_page_fresh_counter">
				<input type="hidden" name="numberOfElementsInArray" value="'.$numberOfMemesInFresh.'">';?>
				<input type="hidden" name="data_array" value='<?php echo base64_encode(serialize($fresh_gaming)) ; ?>'>
			<?php
			echo '
				<button type="submit" class="btn load-more">Load More</button>
			</form>
