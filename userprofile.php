...
echo '<h1 class="info-subscribers">Subscribers:</h1>
	 	<h1 id="'.$subscribeLabel.'" name="'.$subscribeLabel.'" class="info-subscribers" style="margin-left:180px;">'.$numberofSubscribers.'</h1>
	  	<h1 name="Error'.$imageId.'" class="error-message"></h1>
	 	';
echo "<h1 class='usernick'>".htmlentities($name)."</h1>";
echo "<h1 class='info-subscribed'>Subscriptions: ".$otherChannelSubscriptions."</h1>";
echo "<h1 class='info-posts'>Posts: ".$memesUploaded."</h1>";
echo "<h1 class='info-questions'>Questions: ".$numberOfQuestionsAsked."</h1>";
...
<div class="tab-content">
	...
	<div class="tab" id="sociallife">
		...
		<div class="friends_div">
			<h3 class="friend-list-header">My Friends:</h3>
		...
		<div class="notifications-column-userprofile notifications-column" id="user-prof-notif">
			...
			{	
				...
				while($row=mysqli_fetch_assoc($result)){
					...
					if($row['notificationType']=="imageLike"){
						$href="imagedisplay.php?id=".$row['notificationForEventId'];
					}
					echo $href;
					//echo '<p>'.getTime($datetime).'</p>';	
					echo '<a href="'.$nL.'" class="notif-link">	
					      <div class="notif">
							<img src="'.$ppL.'" class="notif-dp">
						    <p class="notif-text userprof-notif-text">'.$notification.'</p>							
						  </div>
