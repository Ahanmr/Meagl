...
<?php
	...
	echo '<img src="defaults/sports_memes.png" style="width=200px;height:200px;" id="group_dp">';
	echo '<h4 class="group-name" style="margin-left:65px;">Sports Memes</h4>';

	$sql="SELECT totalSubscribers,totalMemesForThisCategory FROM memecategoriestable WHERE memeCategory='Sports'";
	$result=mysqli_query($conn,$sql);
	$row=mysqli_fetch_assoc($result);
	$memeCategorySubscribers=$row['totalSubscribers'];
	$totalMemesForThisCategory=$row['totalMemesForThisCategory'];

	$memeCategorySubscribersLabel="categorySubscribeSports";
	...
	if(isset($_SESSION['id'])){
		...
		$sql6= "SELECT subscribedByUserId FROM sportsmemessubscriberstable WHERE subscribedByUserId='$userId'";
	...
	echo '<button type="submit" onclick="memeCategorySubscribeFunction(0,\'Sports\',\''.$memeCategorySubscribersLabel.'\');" name="subscribecategorySports" class="btn" id="userprofile-subscribe-button" style="margin-top:50px;margin-left:10px">'.$subscribeToMemeCategoryInnerHtml.'</button>';
	echo '</div>';
?>
</div>
<div class="tab-content" id="tabs">
	...
	<div class="tab trendingmemes" id="trending" style="right:400px;top:120px">
		<div id="trending_content_div">
			<?php
			$sql="SELECT * FROM memestable WHERE (memeCategory='Sports' AND (visibilityStatus=1 OR visibilityStatus=3)) ORDER BY likes DESC";
			$result=mysqli_query($conn,$sql);
			...
			foreach($sports_trending as $key => $am){
				...
				$sports_trending[$key]['trending_score']=$score;			
			}

			//removing all duplicate values
			$sports_trending = removeDuplicateArrayEntry($sports_trending, 'id');

			//sorting in descending order according to the score
			usort($sports_trending, function ($a, $b) {
			...
			//print_r($sports_trending);

			$_SESSION['sports_page_trending_counter']=0;
			$numberOfMemesInTrending=count($sports_trending);			
			//echo $numberOfMemesInTrending;
			loadMorefunction($sports_trending,$numberOfMemesInTrending,'sports_page_trending_counter','trending',"donthideform");

			echo '</div>';//closing trending_content_div

			//load more button
			if($numberOfMemesInTrending!=$_SESSION['sports_page_trending_counter'])
			{
				echo '
				<form class="load_more_memes_form" id="sports_page_trending_loadmore_form">
					<input type="hidden" name="load_more_type" value="trending">
					<input type="hidden" name="session_counter_name" value="sports_page_trending_counter">
					<input type="hidden" name="numberOfElementsInArray" value="'.$numberOfMemesInTrending.'">';?>
					<input type="hidden" name="data_array" value='<?php echo base64_encode(serialize($sports_trending)) ; ?>'>
				<?php
				echo '
	...
	<div class="tab freshmemes" id="fresh" style="right:400px;top:120px">
		...
		<?php
		$sql= "SELECT * FROM memestable WHERE memeCategory='Sports' AND (visibilityStatus=1 OR visibilityStatus=3) ORDER BY id DESC";//ordering in the descending order(that is, printing all the memes in the decreasing order of their id)
		$result=mysqli_query($conn,$sql);
		...
		if(isset($fresh_sports)){

		$_SESSION['sports_page_fresh_counter']=0;
		$numberOfMemesInFresh=count($fresh_sports);				
		loadMorefunction($fresh_sports,$numberOfMemesInFresh,'sports_page_fresh_counter','fresh',"donthideform");

		echo '</div>';//closing fresh_content_div

		//load more button
		if($numberOfMemesInFresh!=$_SESSION['sports_page_fresh_counter'])
		{
			echo '
			<form class="load_more_memes_form" id="sports_page_fresh_loadmore_form" method="post">
				<input type="hidden" name="load_more_type" value="fresh">
				<input type="hidden" name="session_counter_name" value="sports_page_fresh_counter">
				<input type="hidden" name="numberOfElementsInArray" value="'.$numberOfMemesInFresh.'">';?>
				<input type="hidden" name="data_array" value='<?php echo base64_encode(serialize($fresh_sports)) ; ?>'>
			<?php
			echo '
				<button type="submit" class="btn load-more">Load More</button>
			</form>
