...
<?php
	...
	echo '<img src="defaults/politics_memes.png" style="width=200px;height:200px;" id="group_dp">';
	echo '<h4 class="group-name" style="margin-left:65px;">Politics Memes</h4>';

	$sql="SELECT totalSubscribers,totalMemesForThisCategory FROM memecategoriestable WHERE memeCategory='Politics'";
	$result=mysqli_query($conn,$sql);
	$row=mysqli_fetch_assoc($result);
	$memeCategorySubscribers=$row['totalSubscribers'];
	$totalMemesForThisCategory=$row['totalMemesForThisCategory'];

	$memeCategorySubscribersLabel="categorySubscribePolitics";
	...
	if(isset($_SESSION['id'])){
		...
		$sql6= "SELECT subscribedByUserId FROM politicsmemessubscriberstable WHERE subscribedByUserId='$userId'";
	...
	echo '<button type="submit" onclick="memeCategorySubscribeFunction(0,\'Politics\',\''.$memeCategorySubscribersLabel.'\');" name="subscribecategoryPolitics" class="btn" id="userprofile-subscribe-button" style="margin-top:50px;margin-left:10px">'.$subscribeToMemeCategoryInnerHtml.'</button>';
	echo '</div>';
?>
</div>
<div class="tab-content" id="tabs">
	...
	<div class="tab trendingmemes" id="trending" style="right:400px;top:120px">
		<div id="trending_content_div">
			<?php
			$sql="SELECT * FROM memestable WHERE (memeCategory='Politics' AND (visibilityStatus=1 OR visibilityStatus=3)) ORDER BY likes DESC";
			$result=mysqli_query($conn,$sql);
			...
			foreach($politics_trending as $key => $am){
				...
				$imgId=mysqli_real_escape_string($conn,$am['id']);
				...
				$politics_trending[$key]['trending_score']=$score;			
			}

			//removing all duplicate values
			$politics_trending = removeDuplicateArrayEntry($politics_trending, 'id');

			//sorting in descending order according to the score
			usort($politics_trending, function ($a, $b) {
			...
			//print_r($politics_trending);

			$_SESSION['politics_page_trending_counter']=0;
			$numberOfMemesInTrending=count($politics_trending);			
			//echo $numberOfMemesInTrending;
			loadMorefunction($politics_trending,$numberOfMemesInTrending,'politics_page_trending_counter','trending',"donthideform");

			echo '</div>';//closing trending_content_div

			//load more button
			if($numberOfMemesInTrending!=$_SESSION['politics_page_trending_counter'])
			{
				echo '
				<form class="load_more_memes_form" id="politics_page_trending_loadmore_form">
					<input type="hidden" name="load_more_type" value="trending">
					<input type="hidden" name="session_counter_name" value="politics_page_trending_counter">
					<input type="hidden" name="numberOfElementsInArray" value="'.$numberOfMemesInTrending.'">';?>
					<input type="hidden" name="data_array" value='<?php echo base64_encode(serialize($politics_trending)) ; ?>'>
				<?php
				echo '
	...
	<div class="tab freshmemes" id="fresh" style="right:400px;top:120px">
		...
		<?php
		$sql= "SELECT * FROM memestable WHERE memeCategory='Politics' AND (visibilityStatus=1 OR visibilityStatus=3) ORDER BY id DESC";//ordering in the descending order(that is, printing all the memes in the decreasing order of their id)
		$result=mysqli_query($conn,$sql);
		...
		if(isset($fresh_politics)){

		$_SESSION['politics_page_fresh_counter']=0;
		$numberOfMemesInFresh=count($fresh_politics);				
		loadMorefunction($fresh_politics,$numberOfMemesInFresh,'politics_page_fresh_counter','fresh',"donthideform");

		echo '</div>';//closing fresh_content_div

		//load more button
		if($numberOfMemesInFresh!=$_SESSION['politics_page_fresh_counter'])
		{
			echo '
			<form class="load_more_memes_form" id="politics_page_fresh_loadmore_form" method="post">
				<input type="hidden" name="load_more_type" value="fresh">
				<input type="hidden" name="session_counter_name" value="politics_page_fresh_counter">
				<input type="hidden" name="numberOfElementsInArray" value="'.$numberOfMemesInFresh.'">';?>
				<input type="hidden" name="data_array" value='<?php echo base64_encode(serialize($fresh_politics)) ; ?>'>
			<?php
			echo '
				<button type="submit" class="btn load-more">Load More</button>
			</form>
